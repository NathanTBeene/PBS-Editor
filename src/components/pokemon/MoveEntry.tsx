import { X } from "lucide-react";
import type { PokemonMove } from "../../lib/models/Pokemon";
import { usePokedexContext } from "../../lib/providers/PokedexProvider";
import Autocomplete from "../ui/Autocomplete";
import { useState, useEffect } from "react";
import React from "react";
import InputField from "../ui/InputField";

interface MoveEntryProps {
  move: PokemonMove;
  onMoveChange: (move: PokemonMove) => void;
  onRemove: () => void;
  useLevel?: boolean;
  onFocus?: () => void;
  onBlur?: () => void;
}

const MoveEntry = ({
  move,
  onMoveChange,
  onRemove,
  useLevel = true,
}: MoveEntryProps) => {
  const { moves } = usePokedexContext();
  const [moveName, setMoveName] = useState<string>(move.name);
  const [moveLevel, setMoveLevel] = useState<string>(String(move.level));

  useEffect(() => {
    setMoveName(move.name);
    setMoveLevel(String(move.level));
  }, [move]);

  return (
    <div
      tabIndex={-1}
      className="flex items-center gap-1 p-1 py-2 shadow-md rounded-sm bg-slate-700/50 text-sm min-w-0"
    >
      {useLevel && (
        <div className="w-12 flex-shrink-0">
          <InputField
            type="number"
            value={moveLevel}
            onChange={(value) => {
              setMoveLevel(value as string);
            }}
            className="w-full px-1 py-2.5 border-r-0 border-l-0 border-t-0 rounded-none border-b border-slate-500 text-xs focus:outline-none text-center focus:ring-transparent focus:border-blue-300"
            min={1}
            max={100}
            placeholder="0"
            onFinished={(value) => {
              setMoveLevel(value as string);
              onMoveChange({ ...move, level: parseInt(value as string) });
            }}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.preventDefault();
                e.currentTarget.blur();
              }
            }}
          />
        </div>
      )}
      <div className="flex-1 min-w-0">
        <Autocomplete
          value={moveName}
          onValueChange={setMoveName}
          options={moves.map((m) => m.id)}
          placeholder="Enter move..."
          inputClass="rounded-none border-b border-t-0 border-l-0 border-r-0 focus:ring-transparent focus:border-blue-300"
          onBlur={() => onMoveChange({ ...move, name: moveName })}
        />
      </div>
      <button
        onClick={onRemove}
        className="p-0.5 text-rose-300 hover:text-rose-500 flex-shrink-0 cursor-pointer"
      >
        <X className="w-4 h-4" />
      </button>
    </div>
  );
};

export default React.memo(MoveEntry);
